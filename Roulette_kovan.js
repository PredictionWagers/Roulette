...
var gcsContractAddress_Roulette = "0x53176A577ab54B8C325E06E25b0697EC615F536B";
...
async function DoCreateNewHouse2() {
    ...
    nMaxPayoutPerSpin = web3.utils.toWei(String(DoSafeDecimals(nMaxPayoutPerSpin * gnBaseChipValue)))
...
async function DoShowDataFromContract() {
    ...
    for (nIndex = 1; nIndex < objHouses.anBalances.length; nIndex++) {
        ...
        nData_MaxPayoutPerSpin = DoSafeDecimals(web3.utils.fromWei(objHouses.anMaxPayoutPerSpins[nIndex]) * gcnEthToTokenMultiplier)
...
async function DoUpdateHouse2() {
    ...
    var nValue = web3.utils.toWei(String(DoSafeDecimals(nDeposit * gnBaseChipValue)))
    var nMaxPayoutPerSpin = web3.utils.toWei(String(DoSafeDecimals(nMaxPayoutPerSpin * gnBaseChipValue)))
    ...
    try {
        if (window.ethereum) {
            ...
            if (gsCurrentNetwork == gsGoodMetamaskNetwork && nNetworkID == gnGoodNetworkID) {
                gobjContract_Roulette = new web3.eth.Contract(gobjAbi_Roulette_kovan, gcsContractAddress_Roulette)
