...
var gsaSingles, gsaThirds, gsaColumns, gbaAllSpaces, gnHouseBalance, gnHouseMaxPayoutPerSpin, gbSendtransactionError, gnPublicMaxPayoutPerSpin, gnSpin_WinningNumber, gobjMakeCoinContract
...
async function DoCreateNewHouse2() {
    ...
    if (nDeposit > gnUserETHBalance) {
        alert("You may not deposit more Tokens than your Balance.")
        return
    }
    ...
    var nValue = String(DoSafeDecimals(nDeposit * gcnEthToTokenMultiplier))
    var nMaxPayoutPerSpin = String(DoSafeDecimals(nMaxPayoutPerSpin * gcnEthToTokenMultiplier))
    var objData = gobjContract_Roulette.methods.zCreateHouse(
        web3.utils.asciiToHex(sUsername),
        nMaxPayoutPerSpin,
        nValue
    ).encodeABI()
    await DoSendSignedTransaction(objData, gcsContractAddress_Roulette);
    DoReloadPage()
}
...
async function DoShowDataFromContract() {
    ...
    for (nIndex = 1; nIndex < objHouses.anBalances.length; nIndex++) {
        nData_Balance = DoSafeDecimals(objHouses.anBalances[nIndex] / gcnEthToTokenMultiplier)
        nData_MaxPayoutPerSpin = DoSafeDecimals(objHouses.anMaxPayoutPerSpins[nIndex] / gcnEthToTokenMultiplier)
...
async function DoUpdateHouse2() {
    ...
    var nValue = String(DoSafeDecimals(nDeposit * gcnEthToTokenMultiplier))
    var nMaxPayoutPerSpin = String(DoSafeDecimals(nMaxPayoutPerSpin * gcnEthToTokenMultiplier))
    var nWithdrawal = String(DoSafeDecimals(nWithdrawal * gcnEthToTokenMultiplier))
...
async function DoShowHouseReport() {
    ...
    for (nIndex = 0; nIndex < objEvents.length; nIndex++) {
        nData_CreditOrDebit = DoSafeDecimals(objEvents[nIndex].returnValues.CreditOrDebit / gcnEthToTokenMultiplier)
...
async function DoRefreshMetamask() {
    ...
    try {
        if (window.ethereum) {
            ...
            if (gsCurrentNetwork == gsGoodMetamaskNetwork && nNetworkID == gnGoodNetworkID) {
                gobjContract_Roulette = new web3.eth.Contract(gobjAbi_Roulette_milli, gcsContractAddress_Roulette)
                gobjMakeCoinContract = new web3.eth.Contract(gobjAbi_MILLI_MakeCoin, gcsContractAddress_MakeCoin)

                gnUserETHBalance = DoSafeDecimals(parseFloat(await gobjMakeCoinContract.methods.balanceOf(gsUserETHAccount).call()) / gcnEthToTokenMultiplier)
                gsFormattedUserETHBalance = parseFloat(gnUserETHBalance.toFixed(2)).toLocaleString('en', { minimumFractionDigits: 2 })
                var nApprovedTokens = await gobjMakeCoinContract.methods.allowance(gsUserETHAccount, gcsContractAddress_Roulette).call()

                if (gnUserETHBalance > 0 && nApprovedTokens > 100000000000000) {
                    gnMetaMaskOK = 1
                    ...
                    da.idSpanMetaMaskStatus.innerHTML = "Your <b>" + gsNetwork_Name + "</b> Account (" +
                        ...
                        gsFormattedUserETHBalance + "</b></font> in <b>MILLI</b> tokens."
                }
            }
        }
    ...
    if (gnMetaMaskOK == 0) {
        da.idDivAllDivs.style.display = "none"
        if (gnUserETHBalance > 0) {
            da.idSpanMetaMaskStatus.innerHTML = "Your <b>" + gsNetwork_Name + "</b> Account is: " +
                "<a target=_ex href='https://" + gsNetwork_prefix + "etherscan.io/address/" + gsUserETHAccount +
                "' class=bluelink>" + gsUserETHAccount.substring(0, 5) + "..</a></b> " +
...
*/
    ...
    try {
        ...
        nValue = String(DoSafeDecimals(gnaSpacesClicked.length * gcnEthToTokenMultiplier * da.idCmbChipValue.value))
        DoSubdivideArrays()

        var objData = gobjContract_Roulette.methods.zSpinWheel(
            String(DoSafeDecimals(gcnEthToTokenMultiplier * da.idCmbChipValue.value)),
            da.idCmbHouse.value,
            gbaAllSpaces,
            nValue
        ).encodeABI()
        ...
        if (!gbSendtransactionError) {
            ...
            gnSpin_WinningNumber = parseInt(objResult)
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
async function DoSendSignedTransaction(objData, sDestContract) {
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
async function DoUnlockTokens() {
    var objData = gobjMakeCoinContract.methods.approve(
        gcsContractAddress_Roulette
        , -1
    ).encodeABI()
    await DoSendSignedTransaction(objData, gcsContractAddress_MakeCoin);
    location.reload();
}
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
